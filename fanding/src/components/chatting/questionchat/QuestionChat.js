...
export default class QuestionChat extends Component {
  state = {
    ...
    chongdaeUserEmail: null,
    chongdaeUserNickname: null,
    user: [],
    blockList: [],
    chatDocid: null,
  };
  buildId = async () => {
    // console.log(this.props.chongdaeUserEmail);
    return [this.state.chongdaeUserEmail, this.state.email].sort().join(":");
  };
  ...
  getChongdaeUserData = () => {
    firestore
      .collection("users")
      .where("user_email", "==", this.props.funding.user_email)
      .onSnapshot((snapshot) => {
        var email = snapshot.docs.map((docs) => docs.data().user_email);
        this.setState({ chongdaeUserEmail: email });
        var nickname = snapshot.docs.map((docs) => docs.data().nickname);
        this.setState({ chongdaeUserNickname: nickname });
      });
  };
      ...
      <div style={{ backgroundColor: "#F8F9FA", height: "100vh" }}>
        <div className="container-fluid  p-3">
          <ChatList
            ...
            chongdaeUserEmail={this.state.chongdaeUserEmail}
            chongdaeUserNickname={this.state.chongdaeUserNickname}
